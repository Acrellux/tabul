...
function App() {
  ...
  const handleMouseMove = (e) => {
    ...
    // Update the start position for x and y
    setStartX(e.clientX);
    setStartY(e.clientY);

    // Move the timeline's vertical position based on mouse drag (y movement)
  ...
  return (
    ...
    >
      ...
      <div
        className="grid"
        style={{
          transform: `translate(${gridX}px, ${cameraY}px)`, // Move the grid with the camera for both X and Y
          backgroundPosition: `${-gridX}px ${-cameraY}px`, // Use backgroundPosition only for infinite effect
        }}
      ></div>
